...
function buildP(webpackConfig) {
  return new Promise(function initBuild(resolve, reject) {
    build(webpackConfig, function onDone(error, result) {
...
function watch(webpackConfig, onDone) {
  ...
  async function rebuildOnChange(path) {
    try {
      ...
      let entry = undefined
      try {
        entry = await readEntry(dirname, path)
      ...
      if (!findRenderFunction(entry, publics)) return
...
function externalize(pathPrefix, requiredPath, done) {
  if (pt.isAbsolute(requiredPath) || /^[.]/.test(requiredPath)) {
    // Inline
